...
class FishControllerTest {
    ...
    private FishResponseDTO sampleResponse() {
        return new FishResponseDTO(1L, "Tucunaré", "Peixe predador de água doce",
    ...
    void findAll_returns200WithPage() throws Exception {
        when(fishService.findAll(any(Pageable.class)))
    ...
    void update_validBody_returns200() throws Exception {
        FishResponseDTO updated = new FishResponseDTO(1L, "Dourado", "Peixe de água doce",
            "Rio Paraná", FishType.FRESHWATER, new IconDTO("/icons/dourado.png"),
            OffsetDateTime.now(), OffsetDateTime.now());

        when(fishService.update(eq(1L), any())).thenReturn(updated);

        FishRequestDTO request = new FishRequestDTO("Dourado", "Peixe de água doce",
            "Rio Paraná", FishType.FRESHWATER, new IconDTO("/icons/dourado.png"));
    ...
    void update_fishDoesNotExist_returns404() throws Exception {
        when(fishService.update(eq(99L), any()))
            .thenThrow(new ResourceNotFoundException("Fish not found with id: 99"));
    ...
    void delete_fishDoesNotExist_returns404() throws Exception {
        doThrow(new ResourceNotFoundException("Fish not found with id: 99"))
            .when(fishService).delete(99L);

        mockMvc.perform(delete("/api/fish/99"))
            .andExpect(status().isNotFound());
    }
}
