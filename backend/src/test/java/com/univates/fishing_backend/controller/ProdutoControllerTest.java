...
class ProdutoControllerTest {
    ...
    private ProdutoResponseDTO responseExemplo() {
        return new ProdutoResponseDTO(
                1L, "Teclado", "Mecânico", new BigDecimal("349.90"), 10, true, OffsetDateTime.now(), null);
    }

    @Test
    void listarTodos_retorna200ComPage() throws Exception {
        when(produtoService.listarTodos(any(Pageable.class))).thenReturn(new PageImpl<>(List.of(responseExemplo())));
    ...
    void criar_bodyValido_retorna201() throws Exception {
        ProdutoRequestDTO request = new ProdutoRequestDTO("Teclado", null, new BigDecimal("349.90"), 10);
    ...
    void atualizar_bodyValido_retorna200() throws Exception {
        ProdutoUpdateDTO request = new ProdutoUpdateDTO("Teclado Pro", null, new BigDecimal("399.90"), 8, true);

        ProdutoResponseDTO atualizado = new ProdutoResponseDTO(
                1L, "Teclado Pro", null, new BigDecimal("399.90"), 8, true, OffsetDateTime.now(), OffsetDateTime.now());
    ...
    void deletar_produtoNaoExiste_retorna404() throws Exception {
        ...
        mockMvc.perform(delete("/api/produtos/99")).andExpect(status().isNotFound());
    }
}
