...
class CatchPhotoServiceTest {
    ...
    @Mock private CatchRepository catchRepository;
    @Mock private CatchPhotoRepository catchPhotoRepository;
    @Mock private CatchPhotoStorageService storageService;

    @InjectMocks private CatchPhotoService catchPhotoService;
    ...
    void uploadPhotos_ownerSavesMetadataAndReturnsDtos() {
        ...
        var result = catchPhotoService.uploadPhotos(1L, "owner@fishing.local", List.of(
        ...
        assertThat(captor.getAllValues()).extracting(CatchPhoto::getFilePath)
    ...
    void deletePhoto_ownerDeletesFileAndRow() {
        CatchRecord record = sampleRecord();
        CatchPhoto photo = CatchPhoto.builder().id(20L).catchRecord(record).filePath("photo-1.jpg").position(0).build();
        when(catchRepository.findById(1L)).thenReturn(Optional.of(record));
    ...
    private CatchRecord sampleRecord() {
        return CatchRecord.builder()
            .id(1L)
            .user(User.builder().id(10L).email("owner@fishing.local").name("Owner").password("x").role(Role.USER).active(true).build())
            .waterBody(WaterBody.builder().id(2L).name("Guaíba").waterType(WaterType.RIVER).source("OSM").build())
            .location(new org.locationtech.jts.geom.GeometryFactory().createPoint(new org.locationtech.jts.geom.Coordinate(-51.0, -30.0)))
            .locationVisibility(LocationVisibility.RIVER_ONLY)
            .species(Fish.builder().id(3L).name("Tucunaré").type(FishType.FRESHWATER).build())
